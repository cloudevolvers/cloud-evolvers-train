import { ArrowRight, Blocks, Building2, GraduationCap, ShieldCheck, Users2 } from "lucide-react";
...
export function HomeProofSection() {
    ...
    return (
        <section className="relative overflow-hidden bg-background py-24 sm:py-28">
            ...
            <div className="absolute left-[12%] top-10 h-48 w-48 rounded-full bg-emerald-500/8 blur-3xl" />
            <div className="absolute bottom-0 right-[10%] h-56 w-56 rounded-full bg-sky-500/8 blur-3xl" />

            <div className="container relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
                    <div className="rounded-[2rem] border border-border bg-[linear-gradient(165deg,rgba(255,255,255,0.96),rgba(244,247,250,0.92))] p-8 shadow-sm dark:bg-[linear-gradient(165deg,rgba(18,23,36,0.96),rgba(9,14,24,0.98))] sm:p-10">
                        <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700 dark:text-emerald-300">
                            ...
                            <span>{marketing?.proofLabel || "Why teams book training with us"}</span>
                        </div>

                        <h2 className="font-display mt-5 max-w-2xl text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">
                        ...
                        <div className="mt-8 grid gap-4">
                            {deliveryPoints.map(({ icon: Icon, title, description }) => (
                                <div
                                    key={title}
                                    className="grid gap-4 rounded-[1.5rem] border border-border/80 bg-background/80 p-5 dark:bg-white/[0.03] sm:grid-cols-[auto_1fr]"
                                >
                                    <div className="w-fit rounded-2xl bg-emerald-500/10 p-3 text-emerald-700 dark:text-emerald-300">
                    ...
                    <div className="flex flex-col gap-5">
                        <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#091223] text-white shadow-[0_24px_70px_rgba(5,8,22,0.3)]">
                            ...
                            <div className="grid gap-px bg-white/10 sm:grid-cols-3">
                                {trustStats.map(({ value, label }) => (
                                    <div key={label} className="bg-[#0d172b] px-6 py-5">
                        ...
                        <Link
                            to="/contact"
                            className="group rounded-[2rem] border border-border bg-card/90 p-6 transition-colors hover:border-neutral-400 dark:hover:border-white/20 sm:p-7"
                        >
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <ArrowRight className="mt-1 h-5 w-5 shrink-0 text-foreground transition-transform group-hover:translate-x-1" />
                            </div>
                        </Link>
                    </div>
                </div>
