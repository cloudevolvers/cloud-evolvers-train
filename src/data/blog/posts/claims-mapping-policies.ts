...
export const claimsMappingPoliciesPost: BlogPost = {
  ...
  content: {
    introduction: {
      en: 'Entra ID gives you two ways to customize token claims. The first is optional claims, configured in the application manifest — straightforward but limited to predefined attributes. The second is claims mapping policies, which let you define transformation logic, conditional emission, and custom claim schemas through the Microsoft Graph API or PowerShell. Claims mapping policies are more powerful but also more complex to manage because they exist as separate policy objects that get assigned to service principals, not app registrations. This distinction matters and causes confusion.',
      nl: 'Entra ID biedt je twee manieren om tokenclaims aan te passen. De eerste is optional claims, geconfigureerd in het applicatiemanifest — eenvoudig maar beperkt tot voorgedefinieerde attributen. De tweede is claims mapping policies, waarmee je transformatielogica, conditionele uitgifte en custom claimschema\'s kunt definiëren via de Microsoft Graph API of PowerShell. Claims mapping policies zijn krachtiger maar ook complexer om te beheren omdat ze bestaan als aparte beleidsopjecten die worden toegewezen aan service principals, niet aan app-registraties. Dit onderscheid is belangrijk en veroorzaakt verwarring.'
    },
    sections: [
      {
        title: {
  ...
  "isOrganizationDefault": false
    ...
    conclusion: {
      en: 'Claims mapping policies fill the gap between simple optional claims and a full-blown custom token service. They are the right tool when you need string transformations, conditional emission, or custom group filtering that the app manifest cannot express. Set acceptMappedClaims, define your policy through Graph, assign it to the service principal, and verify the token output at jwt.ms. Just keep the implicit flow limitation in mind and avoid mixing optional claims with policy claims on the same attribute.',
      nl: 'Claims mapping policies vullen de kloof tussen simpele optional claims en een volwaardige custom token service. Ze zijn het juiste instrument wanneer je stringtransformaties, conditionele uitgifte of custom groepsfiltering nodig hebt die het app-manifest niet kan uitdrukken. Stel acceptMappedClaims in, definieer je beleid via Graph, wijs het toe aan de service principal en verifieer de tokenoutput op jwt.ms. Houd alleen de implicit flow-beperking in gedachten en vermijd het mengen van optional claims met beleidsclaims op hetzelfde attribuut.'
    }
  }
};
